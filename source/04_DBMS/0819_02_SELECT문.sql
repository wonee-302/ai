...
DESC EMP; 
    -- EMP 테이블 구조 
SELECT EMPNO,ENAME,SAL HIREDATE FROM EMP; --EMP 테이블에서 EMPNO,ENAME,SAL 열만 검색
SELECT EMPNO AS "사번",ENAME AS "이름" ,SAL AS "급여", HIREDATE AS "입사일" FROM EMP;
SELECT EMPNO  "사번",ENAME  "이름" ,SAL  "급여", HIREDATE  "입사일" FROM EMP;
SELECT EMPNO  "사 번",ENAME ,SAL  급여, HIREDATE  입사일 FROM EMP;


-- 3. 특정 행만 출력 : WHERE 절 (조건절)에서 비교연산자 - 같다(=), 다르다(!=, ^=, <>),>, <, >=, <=
SELECT EMPNO NO, ENAME NAME, SAL FROM EMP WHERE SAL=3000;
SELECT EMPNO NO, ENAME NAME, SAL FROM EMP WHERE SAL^=3000;
SELECT EMPNO NO, ENAME NAME, SAL FROM EMP WHERE SAL>3000;
    -- 비교 연산자는 숫자, 문자, 날짜 모두 가능
    ...
    SELECT * FROM EMP WHERE TO_CHAR(HIREDATE, 'RR/MM/DD')<'81/01/01';
    SELECT * FROM EMP
    WHERE HIREDATE <TO_DATE ('81/01/01', 'RR/MM/DD');
    ALTER SESSION SET NLS_DATE_FORMAT = 'RR/MM/DD';

-- 4.특정행만 출력 :WHERE절 (조건절)에서 논리 연산자: OR, AND, NOT
    -- EX1. 그병(SAL)가 2000부터 3000인 직원의 모든 필드 
...
-- 5. 산술연산자 (SELECT 절, WHERE 절, ORDER BY 절) 
    -- EX. 연봉이 2400이상인 직원이ㅡ ENAME, SAL, 연봉(SAL*12)
    SELECT ENAME, SAL, SAL*12 연봉   --(3)번째 
        FROM EMP                     --(1)번째
        WHWERE SAL*12>=2400          --(2)번째
        ORDER BY 연봉 DESC; -- 연봉순 오름차순(ASC)정렬 ---(4)번째 
    -- EX. 연봉이 10000이상인 직원이ㅡ ENAME, SAL, COMM, 연봉(SAL*12+COMM)
...
SELECT ENAME || '은 ' || JOB FROM EMP; 
    -- EX. 모든 사우너에 대해서 "SMITH"의 연봉은 XXXX$입니다" 라고 출력하고 TITLE에는 MESSAGE. 
...
SELECT EMPNO, ENAME, JOB, HIREDATE FROM EMP WHERE HIREDATE>=TO_DATE('81/03/01','RR/MM/DD');
...
-- 8. SQL 연산자 (BETWEEN, IN, LIKE, IS NULL)
 -- (1) 필드명 BETWEEN A AND B :필드명이 A 부터 B까지 (A,B 포함)A<B
        필드명 NOT BETWEEN A AND B :필드명이 A 미만 B초과  (A,B 미포함)A<B
...
-- (2) 필드명 IN (값1, 값2, ...값N):필드명이 값1이거나 값2이거나,...값N인 경우
    --EX.부서코드가 10번이거나 30인 사람의 사번, 이름, 업무, 부서코드 출력
...
-- (4) IS NULL(NULL인지를 검색할 때) 
    ...
    SELECT * FROM EMP WHRER COMM IN (NULL,0); --불가 
    SELECT * FROM EMP WHERE COMM = NULL OR COM=0; -- 불가 
    SELECT * FROM EMP WHERE COMM IS NULL OR OR COMM=0; 
    -- EX. 상여금을 받는 사원의 모든 필드 (COMM이 NULL이 아니고 COMM이 0이 아닌 사원)
    SELECT * FROM EMP WHERE COMM!=NULL AND COMM!=0; --불가 
    SELECT * FROM EMP WHERE COMM IS NOT NULL AND COMM!=0;

-- 9. 정렬(오름차순 ASC, 내림차순 DESC): ORDER BY 절 
SELECT ENAME, SAL, HIREDATE FROM EMP ORDER BY SAL, HIREDATE DESC, ENAME;
 ...
 날짜형 -> 문자형: TO_CHAR(날짜형, '패턴')
                YYYY(년도4자리), RR(년도2자리), YY(년도2자리), MM(월2자리), DD(일 2자리), DY(수), DAY(수요일)
...
SELECT EMPNO, ENAME, SAL*12+NVL(COMM,0)연봉 FROM EMP WHERE SAL*12+NVL(COMM,0)>=24000 ORDER BY 연봉 DESC;
...
WHERE HIREDATE NOT BETWEEN TO_DATE('81/01/01','RR/MM/DD') AND TO_DATE('81/12/31','RR/MM/DD') 
AND JOB ^='SALESMAN';
