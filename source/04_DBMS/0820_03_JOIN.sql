...
SELECT E.*, DNAME, LOC
 ...
 -- EX. 모든 사원의 사번, 이름, JOB, 상사번호, 부서번호, 부서명
 SELECT EMPNO, ENAME, JOB, MGR, E.DEPTNO, DNAME
   FROM EMP E, DEPT D
   WHERE E.DEPTNO=D.DEPTNO;

 --EX. 급여가 2000 이상인 직원의 이름, 직책, 급여, 부서명, 근무지 출력
 ...
 WHERE LOC='CHICAGO' AND E.DEPTNO=D.DEPTNO;
  -- EX. 상여금(COMM)이 있는 사원 중, SAL이 1200 이상인 사원의 이름, 급여., 부서번호, 부서명, 상여금(부서명순, 급여큰순) 
 ...
 --탄탄EX. ACCOUNTING 부서 소속 사원의 이름과 입사일순으로 출력하시오
...
-- EX. 20번 부서 외의 사원의 사원의 이름, 급여, 부서코드, 근무지를 부서코드순으로 출력하시오. 
SELECT ENAME, SAL, E.DEPTNO, LOC FROM EMP E, DEPT D
WHERE E.DEPTNO=D.DEPTNO AND E.DEPTNO!=20
ORDER BY D.DEPTNO;


--▶ 2.NON-EQUIT JOIN: 동일한 컬럼없이 다른 조건으로 조인 
...
SELECT ENAME, SAL, GRADE, HIREDATE, LOC
    ...
    WHERE SAL BETWEEN LOSAL AND HISAL AND E.DEPTNO=D.DEPTNO AND TO_CHAR(HIREDATE,'RR')=81;
...
SELECT W.ENAME ||'의 상사는 ' || M.ENAME || '다' 매니저
    FROM EMP W, EMP M
    WHERE W.MGR=M.EMPNO;
...
--▶ 4. OUTER JOIN : EQUI-JOIN, NON-EQUI-JOIN, SELF-JOIN시 조건에 만족하지 않는 행까지 나타나게 하는조인
--(1) SELF-JOIN 에서의 OUTER JOIN : 조건에 배제된 행을 결과에 포함 시킬 경우, +기호를 정보가 부족한 컬럼에 붙임
...
SELECT * FROM EMP E, DEPT D
    WHERE E.DEPTNO(+)=D.DEPTNO; -- 40번 부서 출력 됨
    
-- ★ <연습문제>
-- Part1(EQUI JOIN, NON EQUI JOIN)
--1. 모든 사원에 대한 이름, 부서번호, 부서명을 출력하는 SELECT 문장을 작성하여라.
SELECT W.ENAME,W.DEPTNO,D.DNAME FROM EMP W, DEPT D WHERE W.DEPTNO=D.DEPTNO;

--2. NEW YORK에서 근무하고 있는 사원에 대하여 이름, 업무, 급여, 부서명을 출력
SELECT ENAME, JOB, SAL, DNAME FROM EMP W, DEPT D WHERE W.DEPTNO=D.DEPTNO;

--3. 보너스를 받는 사원에 대하여 이름,부서명,위치를 출력
SELECT ENAME, DNAME, LOC FROM EMP W, DEPT D WHERE W.DEPTNO=D.DEPTNO AND COMM IS NOT NULL;

--4. 이름 중 L자가 있는 사원에 대하여 이름,업무,부서명,위치를 출력
SELECT ENAME, JOB, ENAME, LOC FROM EMP W, DEPT D WHERE W.DEPTNO=D.DEPTNO AND ENAME LIKE '%L%';

--5. 사번, 사원명, 급여, 부서명을 출력(단, 급여가 2000이상인 사원에 대하여 급여를 기준으로 내림차순 정렬)
SELECT EMPNO, ENAME, SAL, DNAME FROM EMP W, DEPT D WHERE SAL >=2000 ORDER BY SAL ;

--6. 사번, 사원명, 업무, 급여, 급여등급, 부서명을 출력(단, 업무가 MANAGER이며 급여가 2500이상인
-- 사원에 대하여 사번을 기준으로 오름차순정렬)
SELECT EMPNO, ENAME, JOB, SAL, GRADE, DNAME 
FROM EMP W,SALGRADE S, DEPT D
WHERE W.DEPTNO=D.DEPTNO 
    AND W.SAL BETWEEN S.LOSAL 
    AND S.HISAL AND JOB='MANAGER' 
    AND SAL>=2500 
ORDER BY W.EMPNO;
...
SELECT W.ENAME || '의 급여는 '|| W.SAL ||'이고 업무는'|| W.JOB ||'이며'||'상사의 이름은'|| NVL(M.ENAME, '없음' )||'입니다.' 
FROM EMP W, EMP M WHERE W.MGR=M.EMPNO(+); 

--3. 이름, 급여, 부서명, 직속상사명
SELECT W.ENAME, W.SAL , DNAME,  M.ENAME MANABER
...
SELECT W.ENAME, W.SAL, GRADE, DNAME, M.ENAME FROM EMP W, EMP M , DEPT D,  SALGRADE S
...
SELECT W.ENAME, W.SAL, GRADE, DNAME, ((W.SAL+NVL(W.COMM,0))*12) 연봉 FROM EMP W, EMP M , DEPT D,  SALGRADE S
